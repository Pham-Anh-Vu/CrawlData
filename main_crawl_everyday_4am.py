...
def CrawlMaTinTucDongThau(pageNumber, codes, details, session1, folder_path1):
    ...
    try:
        ...
        for code in codes:
            ...
            if code[4] == 'DHT':
                CrawlDetail_DT_DHT(code=code[0], details=details, session1=session1, codes=code,
                                   folder_path1=folder_path1)

            elif code[4] == 'KCNTTT':
                if code[6] == 1:
                    CrawlDetail_DT_KCNTTT(code=code[0], details=details, session1=session1, codes=code,
                                          folder_path1=folder_path1)
                elif code[6] == 0:
                    CrawDetail_DT_KCNTTT_KQM(inputResultId=code[5], details=details, session1=session1, codes=code,
                                             folder_path1=folder_path1)
                ...
                if code[6] == 1:
                    if code[7] == 'TV':
                        if code[3] == 'notify-contractor-step-2-kqmt':
                            CrawlDetail_DT_DXT_TV_KQMT(code=code[0], details=details, session1=session1, codes=code,
                                                       folder_path1=folder_path1, notify_no=code[1])
                            # Code DXT KQMT
                        elif code[3] == 'notify-contractor-step-3-dsntdkt':
                            CrawlDetail_DT_DXT_TV_DSNTDKT(code=code[0], details=details, session1=session1, codes=code,
                                                          folder_path1=folder_path1, notify_no=code[1])
                            # Code DXT DSNTDXKT
                        else:
                            print(code)
                    else:
                        CrawlDetail_DT_DXT(code=code[0], details=details, session1=session1, codes=code,
                                           folder_path1=folder_path1, notify_no=code[1])
